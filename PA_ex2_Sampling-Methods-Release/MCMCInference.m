  ...
  for i = 1:size([u, v], 1)
      ...
      if u(i) > v(i)
          ...
          elseif strcmp(TransName, 'MHSwendsenWang2')
              ...
              num_vals_idx = assignments(:,1) == assignments(:,2);
              rel_assignments = assignments(num_vals_idx,:);
              num_vals = GetValueOfAssignment(edge_factor,rel_assignments);
              denum_vals = edge_factor.val;
              % for reference look at page4 forumla for q_ij
              
              num = sum(num_vals);
              denum = sum(denum_vals);
              q_ij = num/denum;
              %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
          else
...
for i = 1:max_iter
    ...
    A_prev = all_samples(i,:);
    A_next = Trans(A_prev,G,F);
    all_samples(i+1, :) = A_next; 
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end
